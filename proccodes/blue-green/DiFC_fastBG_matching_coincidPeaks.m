function [ out_dat ] = DiFC_fastBG_matching_coincidPeaks( data_path, data_file, save_flag, save_to, plot_flag)
...
ch1_info_preCoincPk = ch1_info; ch2_info_preCoincPk = ch2_info;
...
%     plot(time(ch1_info_preCoincPk.locs), ch_pp(ch1_info_preCoincPk.locs,1), ' o', 'MarkerSize', 6);
...
mpi(find(mpi == 0)) = []; mpi_rev(find(mpi_rev == 0)) = [];
if plot_flag == 1
    if true_counts>=1
...
out_dat.ch1_pk_cand = ch1_info.locs;
out_dat.ch2_pk_cand = ch2_info.locs;
