function plotAllPeaks3(data, time, peaks, fwd_matches, rev_matches,coinc_peaks, thresh_curve, params,saveDir,phantom,yAxesMax,yAxesMin,FwdorRev)
...
if phantom == 1%For plotting phantom data
    ...
    if FwdorRev
        ...
        for ii = 1:length(data(1,:))
            ...
            %ylabel(['Signal (' params(ii).units ')'])
            %xlabel('Time (s)')
            legend('DiFC Data', 'Threshold', sprintf('Unmatched: %g',unmatchedTotal(ii)), sprintf('Rev: %g',rev_matches(ii).count), sprintf('Coincident: %g',coinc_peaks(ii).count),sprintf('Fwd: %g',fwd_matches(ii).count),'FontSize', 18,'Location','northeastoutside');
            axis = gca;
            axis.FontSize = axisFontSize;
            axis.LineWidth = 2;
            axis.FontWeight = 'bold';
